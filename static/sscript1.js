...
function checkGameOver(data) {
    ...
    if (data.game_over) {
        // alert(`Game Over! Winner: ${data.winner}<br>CLick on OK TO start New Game`);
        disableBoard();
...
function makeBotMove(gameId) {
    // Bot move logic: fetch current game state and decide on a move
    fetch(`/bot_move`, {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.status === 'move made') {
            updateBoard(data);  // Update board with bot's move
            checkGameOver(data);
        } else {
            alert('Failed to make bot move');
        }
    })
    .catch(error => {
        console.error('Error:', error);
        alert('An error occurred while making the bot move.');
    });
}
...
function selectSquare(square) {
    ...
    if (selectedPiece) {
        ...
        .then(data => {
            if (data.status === 'move made') {
                updateBoard(data);  // Make sure this function correctly updates the board with the bot's move
                checkGameOver(data);
                if (data.turn === 'white') {
                        // The bot should make its move
                        makeBotMove(gameId);
